(() => {
    ...
    da.init();
    const strelka = document.querySelectorAll(".success__item");
    let script_value = 1;
    strelka.forEach((function cnt(element, index) {
        if (index < 4) {
            element.classList.add(`success__item_${script_value}`);
            script_value++;
        } else if (index >= 4) {
            script_value = 1;
            strelka.forEach((function cnt(element, index) {
